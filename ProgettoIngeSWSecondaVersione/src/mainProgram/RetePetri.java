...
@XmlType(name = "RetePetri", propOrder = { "marcature" })
@XmlSeeAlso({ RetePetriP.class })
public class RetePetri extends Rete implements ICercaTopologiaBase {
	 ...
	 * marcature: da 0 a +infinito pesi: da 1 a +infinito
	 */
	@Override
	...
	public RetePetri creaRete() {
		if (arch.noRetiInArchivio()) {
			...
			if (r != null) {
				RetePetri p = new RetePetri();
				p.setName(LeggiInput.leggiStringaNonVuota(MESS_NOME));
				r.contaPosizioni();
				p.inizializzaReteP(r);
				System.out.println("----------------------------------------------");
				return p;
			}
		}
	...
	public boolean retiInArchivio() {
		if (arch.reti != null && !(arch.reti.isEmpty()) && !(arch.noRetiPNInArchivio())) {
			return true;
		}
		return false;
	}
	...
	public void aggiungiPesiRelazione(Rete re) {
		for (AbstractRelazioneDiFlusso rel : re.getRelazioni()) {
			System.out.println();
			int peso = LeggiInput
					.leggiInteroPositivo("inserire il peso in questa relazione di flusso [" + rel.toString() + "]: ");
	...
	public void simulaRete() {
		...
		do {
			...
			if (numTransAbil == 1) {
				...
				// cerco con il ciclo nel vettore di booleani la transizione
				// abilitata e la faccio scattare
				...
				// utente sceglie quale delle transizioni abilitate fare
				// scattare
	...
	public ArrayList<AbstractRelazioneDiFlusso> getTopologiaSottostante() {
		...
		for (AbstractRelazioneDiFlusso relP : getRelazioni()) {
			RelazioneDiFlusso relF = new RelazioneDiFlusso(relP.getPosizione(), relP.getTransizione(), relP.isInOut());
			arrayRDF.add(relF);
		}
		return arrayRDF;
	}

	// la rete di Petri da file deve basarsi su una rete già presente
	// nell'archivio
	@Override
